...
class DatasetController extends Controller
{
    ...
    // Download compiled report as offline HTML document
    public function downloadReport($reportId)
    {
        $report = Report::with('creator')->findOrFail($reportId);
        $filename = str_replace(' ', '_', strtolower($report->report_title)) . '.html';
...
<head>
    <meta charset='utf-8'>
    <title>{$report->report_title} - FloraMapper</title>
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333333; margin: 40px; background: #ffffff; }
        .header { border-bottom: 3px solid #1e5631; padding-bottom: 10px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: flex-end; }
        .header-title h1 { margin: 0; color: #1e5631; font-size: 24px; }
        .header-title span { font-size: 13px; color: #666666; font-weight: bold; }
        .meta-info { text-align: right; font-size: 12px; color: #555555; line-height: 1.4; }
        h2, h3 { color: #1e5631; margin-top: 25px; }
        ul { padding-left: 20px; }
        li { margin-bottom: 8px; }
        table { width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 13px; }
        th, td { border: 1px solid #dddddd; padding: 10px; text-align: left; }
        th { background: #f4f6f4; font-weight: bold; }
        tr:nth-child(even) { background: #fafafa; }
        .footer { margin-top: 50px; border-top: 1px solid #eeeeee; padding-top: 15px; font-size: 11px; color: #777777; text-align: center; }
    </style>
</head>
<body>
    <div class='header'>
        ...
        <div class='meta-info'>
            <strong>Generated By:</strong> " . ($report->creator ? $report->creator->full_name : 'System') . "<br>
            <strong>Date Generated:</strong> " . ($report->created_at ? $report->created_at->format('Y-m-d H:i:s') : now()->format('Y-m-d H:i:s')) . "
        </div>
    </div>
    ...
    <div class='footer'>
        &copy; " . date('Y') . " FloraMapper System Kenya. All rights reserved. Generated automatically via KEFRI Analytical Portal.
    </div>
</body>
</html>";
    ...
    // Download region comparison report as offline HTML document
    public function downloadComparison(Request $request)
    {
        $request->validate([
            'region_a' => 'required|exists:regions,region_id',
            'region_b' => 'required|exists:regions,region_id',
        ]);

        $regionA = Region::with(['assessments' => function($q) { $q->latest(); }])->findOrFail($request->region_a);
        $regionB = Region::with(['assessments' => function($q) { $q->latest(); }])->findOrFail($request->region_b);

        $latestA = $regionA->assessments->first();
        $latestB = $regionB->assessments->first();

        $filename = "comparison_" . str_replace(' ', '_', strtolower($regionA->region_name)) . "_vs_" . str_replace(' ', '_', strtolower($regionB->region_name)) . ".html";

        // Helper closures for rendering region blocks in HTML download
        $renderRegionBlock = function($region, $latest) {
            $html = "<h2>" . e($region->region_name) . "</h2>";
            $html .= "<table style='width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 13px;'>";
            $html .= "<tr><td style='padding:8px; font-weight:bold; width:35%; border:1px solid #ddd;'>County:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($region->county) . "</td></tr>";
            $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Ecosystem Type:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($region->ecosystem_type) . "</td></tr>";
            $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Coordinates:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($region->latitude) . ", " . e($region->longitude) . "</td></tr>";
            $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Description:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($region->description ?? 'No description available.') . "</td></tr>";
            $html .= "</table>";

            if ($latest) {
                $html .= "<h3 style='margin-top:20px; color:#1e5631;'>Latest Vulnerability Assessment</h3>";
                $html .= "<table style='width: 100%; border-collapse: collapse; font-size: 13px;'>";
                $html .= "<tr><td style='padding:8px; font-weight:bold; width:35%; border:1px solid #ddd;'>Vulnerability Level:</td><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>" . e($latest->vulnerability_level) . "</td></tr>";
                $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Overall Index Score:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($latest->overall_score) . "%</td></tr>";
                $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Temperature Score:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($latest->temperature_score) . "%</td></tr>";
                $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>Rainfall Score:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($latest->rainfall_score) . "%</td></tr>";
                $html .= "<tr><td style='padding:8px; font-weight:bold; border:1px solid #ddd;'>NDVI Cover Score:</td><td style='padding:8px; border:1px solid #ddd;'>" . e($latest->ndvi_score) . "%</td></tr>";
                $html .= "</table>";

                $html .= "<div style='background:#f9f9f9; border:1px solid #ddd; padding:12px; margin-top:15px; font-size:12px; border-radius:4px; line-height:1.5;'>";
                $html .= "<strong>Interpretation Note:</strong><br>" . e($latest->interpretation);
                $html .= "</div>";
            } else {
                $html .= "<p style='font-style:italic; text-align:center; padding:15px; border:1px dashed #ccc;'>No assessment records compiled for this ecosystem.</p>";
            }

            return $html;
        };

        $blockA = $renderRegionBlock($regionA, $latestA);
        $blockB = $renderRegionBlock($regionB, $latestB);
...
<head>
    <meta charset='utf-8'>
    <title>Ecosystem Comparison Report - FloraMapper</title>
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333333; margin: 40px; background: #ffffff; }
        .header { border-bottom: 3px solid #1e5631; padding-bottom: 10px; margin-bottom: 30px; }
        .header h1 { margin: 0; color: #1e5631; font-size: 24px; }
        .header p { margin: 5px 0 0 0; font-size: 13px; color: #666666; font-weight: bold; }
        .comparison-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-top: 30px; }
        .column { background: #ffffff; border: 1px solid #eaeaea; border-radius: 6px; padding: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.02); }
        h2 { color: #1e5631; margin-top: 0; border-bottom: 2px solid #1e5631; padding-bottom: 8px; }
        .footer { margin-top: 60px; border-top: 1px solid #eeeeee; padding-top: 15px; font-size: 11px; color: #777777; text-align: center; }
    </style>
</head>
<body>
    <div class='header'>
        <h1>Ecosystem Comparison Report</h1>
        <p>Generated by: " . e(Auth::user()->full_name) . " | Date: " . now()->format('Y-m-d H:i:s') . "</p>
    </div>

    <p style='font-size:13px; color:#555;'>This document provides a comparative vulnerability assessment summary between the following two study regions.</p>
