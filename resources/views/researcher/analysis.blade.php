    ...
    <div class="main-content">
        ...
        <div class="panel">
                ...
                <div class="form-group">
                    ...
                    <select id="region_id" name="region_id" class="form-control" required>
                        ...
                        @foreach ($regions as $region)
                            <option value="{{ $region->region_id }}">{{ $region->region_name }} ({{ $region->county }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="climate_dataset_id" name="climate_dataset_id" class="form-control" required>
                        ...
                        @foreach ($climateDatasets as $dataset)
                            <option value="{{ $dataset->dataset_id }}">{{ $dataset->dataset_name }} ({{ $dataset->source_name }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="info-box">
                ...
                <p>When running this evaluation, the system averages the uploaded temperature, rainfall, and NDVI parameters for the selected region and compares them using the following indicators:</p>
                <ul>
                    <li><strong>Temperature Index</strong>: Measures deviation from the ideal forest temperature threshold (20°C). Higher deviations indicate high climate sensitivity.</li>
                    <li><strong>Rainfall Index</strong>: Measures scarcity of rain (inverse scale relative to a 200mm base target).</li>
                    <li><strong>NDVI Index</strong>: Measures vegetation thinness (inverse scale relative to a healthy 0.8 NDVI density).</li>
                </ul>
                <p>The averaged score is checked against the configured low/moderate/high thresholds to save the regional rating.</p>
            </div>
        </div>
    </div>
