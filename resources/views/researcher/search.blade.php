    ...
    <div class="sidebar">
        <div>
            ...
            <ul class="menu-list">
                ...
                <li class="menu-item">
                    <a href="{{ route('researcher.search') }}"
                        class="menu-link{{ request()->routeIs('researcher.search') ? ' active' : '' }}">Search Registry</a>
                </li>
            </ul>
        ...
        <div class="main-content">
            <div class="header">
                ...
                <span style="font-size: 13px; background: #e2e8f0; padding: 4px 8px; border-radius: 4px;">Flora &
                    Ecosystems</span>
            </div>
            ...
            <div class="search-bar-panel">
                <form action="{{ route('public.search') }}" method="GET" class="search-form">
                    <input type="text" name="q" value="{{ $query }}" class="search-input"
                        placeholder="Search by region, county, ecosystem, scientific name, conservation status..."
                        required autofocus>
                ...
                <div class="results-container">
                    ...
                    <div class="panel">
                        ...
                        @forelse ($flora as $species)
                            <div class="result-item">
                                <strong
                                    style="color: #1e5631; font-size: 15px;">{{ $species->scientific_name }}</strong>
                                <span
                                    style="font-style: italic; font-size: 13px; color: #666;">({{ $species->common_name ?? 'No common name' }})</span>
                                <div style="font-size: 12px; color: #555; margin-top: 3px;">
                                ...
                                @endphp
                                <span class="vuln-badge {{ $badgeClass }}">Sensitivity:
                                    {{ $lvl ?? 'Unknown' }}</span>
                            </div>
                        @empty
            ...
            @else
                <div class="panel" style="display: flex; align-items: center; justify-content: center;">
                    <div class="info-msg">
                        Enter keywords above to search for ecosystem regions, vulnerability ratings, and registered
                        flora species.
                    </div>
                </div>
            @endif
