                ...
                .then(data => {
                    ...
                    if (data.flora.length === 0) {
                        ...
                        data.flora.forEach(f => {
                            ...
                            floraHtml += `<li><a href="${googleUrl}" target="_blank" style="color: #1e5631; font-weight: bold; text-decoration: underline;">${f.scientific_name}</a> (${f.common_name || 'N/A'}) - <em>${f.conservation_status || 'Unknown status'}</em></li>`;
                        });
                    }
                    floraHtml += '</ul>';
